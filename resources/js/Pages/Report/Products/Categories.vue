    ...
    <AuthenticatedLayout title="Product Category Report" description="Analyze products by category with detailed breakdowns"
        ...
        <div class="py-6">
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                ...
                <!-- Page Header -->
                <div class="mb-8">
                    <h1 class="text-3xl font-bold text-gray-900 mb-2">Product Category Report</h1>
                    <p class="text-gray-600">Comprehensive analysis of products organized by categories with status breakdowns</p>
                </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <p class="text-2xl font-bold text-gray-900">{{ inactiveCategories }}</p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-for="category in categories.data" :key="category.id" class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        ...
                                        >
                                            {{ expandedCategories.includes(category.id) ? 'Hide' : 'View' }} Products
                                        </button>
                                    </td>
                                </tr>
                            </tbody>
                    ...
                    <div v-for="category in categories.data" :key="`details-${category.id}`" v-show="expandedCategories.includes(category.id)" class="border-t border-gray-200 bg-gray-50">
                        <div class="px-6 py-4">
                            <h4 class="text-sm font-medium text-gray-900 mb-3">Products in {{ category.name }}</h4>
                            <div class="overflow-x-auto">
                                <table class="min-w-full divide-y divide-gray-200">
...
const activeCategories = computed(() => {
    return props.categories.data.filter(category => category.is_active).length;
});
...
const totalProducts = computed(() => {
    return props.categories.data.reduce((sum, category) => sum + category.total_products, 0);
});

const applyFilters = () => {
    router.get(route('reports.products.categories'), filters.value, {
        preserveState: true,
        preserveScroll: true
    });
};
