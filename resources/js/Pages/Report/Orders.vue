<template>
    <AuthenticatedLayout
        title="Orders Report"
        description="Monthly Orders Summary"
    ...
    >
        ...
        <transition name="fade">
            ...
            >
                <div class="bg-white rounded-lg shadow-xl w-full max-w-7xl max-h-[90vh] flex flex-col modal-content">
                    <!-- Modal Header -->
                    <div class="flex items-center justify-between p-4 border-b">
                        ...
                        <button
                            ...
                            class="text-gray-400 hover:text-gray-500 no-print"
                            aria-label="Close"
                        >
                        ...
                        <!-- Approvals -->
                        <div
                            class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm mb-4"
                        >
                            <div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full text-start text-xs border">
                                <thead class="bg-gray-100">
                                    <tr>
                                        <th class="text-left border border-black p-2 w-72">Item</th>
                                        <th class="text-left border border-black p-2 w-20">UOM</th>
                                        <th class="text-left border border-black p-2 w-20">Barcode</th>
                                        <th class="text-left border border-black p-2 w-20">Batch Number</th>
                                        <th class="text-left border border-black p-2 w-20">Expiry Date</th>
                                        <th class="text-left border border-black p-2 w-20">Quantity</th>
                                        <th class="text-left border border-black p-2 w-28">Warehouse</th>
                                        <th class="text-left border border-black p-2 w-28">Location</th>
                                        <th class="text-left border border-black p-2 w-20">Unit Cost</th>
                                        <th class="text-left border border-black p-2 w-20">Total Cost</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr
                                        v-for="allocation in selectedOrder.inventory_allocations"
                                        :key="allocation.id"
                                        class="border-b"
                                    >
                                        <td class="text-left border border-black p-2">{{ allocation.product }}</td>
                                        <td class="text-left border border-black p-2">{{ allocation.uom }}</td>
                                        <td class="text-left border border-black p-2">{{ allocation.barcode }}</td>
                                        <td class="text-left border border-black p-2">{{ allocation.batch_number }}</td>
                                        ...
                                        <td class="text-left border border-black p-2">{{ allocation.quantity }}</td>
                                        <td class="text-left border border-black p-2">{{ allocation.warehouse }}</td>
                                        <td class="text-left border border-black p-2">
                                            {{
            ...
            <div class="mb-6">
                <div class="flex justify-between items-start mb-4">
                    <div>
                        ...
                        <p class="text-gray-600">Generated on: ${formatDate(new Date().toISOString())}</p>
                    </div>
                ...
                <div class="grid grid-cols-2 gap-4 mb-6">
                    <div class="bg-gray-50 p-4 rounded">
                        ...
                        <p><strong>Facility:</strong> ${order.facility?.name || 'N/A'}</p>
                        <p><strong>Type:</strong> ${order.order_type}</p>
                        <p><strong>Order Date:</strong> ${formatDate(order.order_date)}</p>
                        <p><strong>Expected Date:</strong> ${order.expected_date ? formatDate(order.expected_date) : 'N/A'}</p>
                    </div>
                    <div class="bg-gray-50 p-4 rounded">
                        <h3 class="font-semibold mb-2">Approval & Dispatch</h3>
                        <p><strong>Approved By:</strong> ${order.approved_by?.name || 'Not Approved'}</p>
                        <p><strong>Dispatched By:</strong> ${order.dispatched_by?.name || 'Not Dispatched'}</p>
                        <p><strong>Rejected By:</strong> ${order.rejected_by?.name || 'N/A'}</p>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full">
                        ...
                        <tbody>
                            ${order.inventory_allocations.map(item => `
                                <tr>
                                    ...
                                    <td>${item.location?.location || item.location?.name || 'N/A'}</td>
                                    <td class="text-right">${formatCurrency(item.unit_cost || 0)}</td>
                                    <td class="text-right font-medium">${formatCurrency((item.unit_cost || 0) * item.quantity)}</td>
                                </tr>
                            `).join('')}
                        </tbody>
                        <tfoot>
                            <tr>
                                ...
                                <td class="text-right font-semibold">
                                    ${formatCurrency(order.inventory_allocations.reduce((sum, item) => 
