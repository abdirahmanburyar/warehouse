            ...
            <div class="p-6 text-gray-900">
                ...
                <div class="flex flex-col lg:flex-row lg:gap-6">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4 lg:hidden">
                        <div class="bg-orange-400 p-4 rounded-xl hover:shadow-md transition-all duration-200 cursor-pointer text-white" @click="navigateToTab('all')">
                        ...
                        <div class="bg-pink-500 p-4 rounded-xl hover:shadow-md transition-all duration-200 cursor-pointer text-white" @click="navigateToTab('near')">
                            <div class="flex items-start justify-between">
                                <div>
                        ...
                        <div class="bg-gray-500 p-4 rounded-xl hover:shadow-md transition-all duration-200 cursor-pointer text-white" @click="navigateToTab('expired')">
                            <div class="flex items-start justify-between">
                                <div>
                ...
                <div class="border-b border-gray-200 mb-4">
                    <nav class="-mb-px flex space-x-8">
                        ...
                        <div class="mb-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                            ...
                            <div class="flex items-center space-x-4">
                                <select v-model="warehouseFilter" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        <th v-if="currentTab === 'expired' || currentTab === 'near'" scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                            <input type="checkbox" v-model="selectAll" @change="toggleSelectAll">
                                        </th>
                                        <th @click="sort('product_name')" scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer">
                                            Product
                                            <span v-if="sort_field === 'product_name'" class="ml-1">
                                                {{ sort_direction === 'asc' ? '↑' : '↓' }}
                                            </span>
                                        </th>
                                        <th @click="sort('batch_number')" scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer">
                                            Batch Number
                                            <span v-if="sort_field === 'batch_number'" class="ml-1">
                                                {{ sort_direction === 'asc' ? '↑' : '↓' }}
                                            </span>
                                        </th>
                                        <th @click="sort('quantity')" scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer">
                                            Quantity
                                            <span v-if="sort_field === 'quantity'" class="ml-1">
                                                {{ sort_direction === 'asc' ? '↑' : '↓' }}
                                            </span>
                                        </th>
                                        <th @click="sort('manufacturing_date')" scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer">
                                            Manufactured
                                            <span v-if="sort_field === 'manufacturing_date'" class="ml-1">
                                                {{ sort_direction === 'asc' ? '↑' : '↓' }}
                                            </span>
                                        </th>
                                        <th @click="sort('expiry_date')" scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer">
                                            Expires
                                            <span v-if="sort_field === 'expiry_date'" class="ml-1">
                                                {{ sort_direction === 'asc' ? '↑' : '↓' }}
                                            </span>
                                        </th>
                                        <th @click="sort('location')" scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer">
                                            Location
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <tr v-for="inventory in inventories.data" :key="inventory.id" 
                                        ...
                                        <td v-if="currentTab === 'expired' || currentTab === 'near'" class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900">{{ formatDate(inventory.manufacturing_date) }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span :class="getStatusClass(inventory)" class="px-2 inline-flex text-2xs leading-5 font-semibold p-1">
                                                {{ getStatusText(inventory) }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            <button v-if="inventory.is_active && (inventory.is_expired || inventory.is_near_expiry)" 
                                    ...
                                    <tr v-if="inventories.data.length === 0">
                                        <td :colspan="currentTab === 'expired' || currentTab === 'near' ? 9 : 8" class="px-6 py-4 text-center text-gray-500">
                                            No items found.
                                        </td>
                                    </tr>
                                </tbody>
                    ...
                    <div class="hidden lg:grid lg:grid-cols-1 gap-3 lg:w-2/12 lg:order-2 lg:sticky lg:top-0 lg:self-start">
                        <div class="bg-orange-400 p-4 rounded-xl hover:shadow-md transition-all duration-200 cursor-pointer text-white" @click="navigateToTab('all')">
                            <div class="flex items-start justify-between">
                                <div>
                                    <div class="font-medium mb-1">Expiring within next<br/>1 Year</div>
                ...
                <Modal :show="showDisposeModal" @close="showDisposeModal = false">
                    <div class="p-6">
                        ...
                        <p class="mb-4 text-gray-600">
                            You are about to mark {{ selectedInventories.length }} item(s) as disposed. This action cannot be undone.
                        </p>
                        ...
                        <div class="mt-6 flex justify-end space-x-3">
                            ...
                            <PrimaryButton @click="confirmDisposal" :class="{'opacity-25': processing}" :disabled="processing">
