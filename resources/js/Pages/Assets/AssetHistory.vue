<template>
    <AuthenticatedLayout :title="pageTitle" :description="pageDescription">
        <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div class="bg-blue-50 rounded-lg p-4 border border-blue-200">
                        <div class="flex items-center">
                            ...
                            <div class="ml-3">
                                <p class="text-sm font-medium text-blue-600">Total Asset Items</p>
                                <p class="text-2xl font-bold text-blue-900">{{ assetItems.length }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-green-50 rounded-lg p-4 border border-green-200">
                        <div class="flex items-center">
                            ...
                            <div class="ml-3">
                                ...
                                <p class="text-2xl font-bold text-green-900">{{ getTotalHistoryCount() }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-purple-50 rounded-lg p-4 border border-purple-200">
                        <div class="flex items-center">
                            ...
                            <div class="ml-3">
                                <p class="text-sm font-medium text-purple-600">Items with History</p>
                                <p class="text-2xl font-bold text-purple-900">{{ getItemsWithHistoryCount() }}</p>
                            </div>
                        </div>
                    </div>
            ...
            <div class="bg-white shadow-xl rounded-2xl p-6">
                <h2 class="text-xl font-semibold text-gray-900 mb-4">Individual Asset Item History</h2>
                <p class="text-sm text-gray-600 mb-6">
                    Each asset item maintains its own history of actions, transfers, and status changes. 
                    Below you can see the detailed timeline for each individual asset item.
                </p>
                
                <div class="space-y-6">
                    <div v-for="item in assetItems" :key="item.id" class="border border-gray-200 rounded-lg p-6">
                        <!-- Asset Item Header -->
                        <div class="flex items-center justify-between mb-4 pb-3 border-b border-gray-200">
                            <div class="flex items-center space-x-3">
                                ...
                                <div>
                                    <h3 class="text-lg font-semibold text-gray-900">{{ item.asset_tag }}</h3>
                        ...
                        <!-- History Timeline -->
                        <div v-if="item.asset_history && item.asset_history.length > 0" class="space-y-4">
                            <h4 class="text-md font-medium text-gray-700 mb-3">History Timeline</h4>
                            <div class="relative">
                                <!-- Timeline Line -->
                                ...
                                <!-- History Items -->
                                <div class="space-y-4">
                                    <div v-for="(history, index) in item.asset_history" :key="history.id" 
                        ...
                        <div v-else class="text-center py-8 text-gray-500">
                            ...
                            <p class="text-sm font-medium text-gray-900 mb-2">No History Records</p>
                            <p class="text-xs text-gray-500">This asset item has no recorded history yet.</p>
                        </div>
                    </div>
                </div>
            </div>
...
const getTotalHistoryCount = () => {
    return props.assetItems.reduce((total, item) => {
        return total + (item.asset_history ? item.asset_history.length : 0);
    }, 0);
};

const getItemsWithHistoryCount = () => {
    return props.assetItems.filter(item => 
        item.asset_history && item.asset_history.length > 0
    ).length;
};
...
const getLatestActivityDate = () => {
    ...
    props.assetItems.forEach(item => {
        if (item.asset_history && item.asset_history.length > 0) {
            item.asset_history.forEach(history => {
