    ...
    <AuthenticatedLayout>
        ...
        <div class="bg-white mb-[100px]">
            ...
            <div class="space-y-6">
                ...
                <div v-if="$page.props.auth.can.supply_edit" class="bg-white rounded-lg shadow p-6 mb-6">
                    ...
                    <form @submit.prevent="submitDocuments">
                        ...
                        <div class="flex gap-2 mb-4">
                            ...
                            >
                                <svg v-if="uploading" class="animate-spin h-4 w-4 mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
...
const newDocuments = ref([
    { type: '', file: null, po_id: props.po.id }
]);
const docErrors = ref([]);
const uploading = ref(false);

function addDocumentRow() {
    newDocuments.value.push({ type: '', file: null, po_id: props.po.id });
}
...
async function submitDocuments() {
    ...
    newDocuments.value.forEach((doc, idx) => {
        ...
        formData.append(`documents[${idx}][po_id]`, doc.po_id);
    });
    uploading.value = true;
    await axios
        .post(route('purchaseo.documents.store', props.po.id), formData)
        .then((response) => {
        ...
        .catch((error) => {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: error.response?.data || 'Failed to upload documents',
                showConfirmButton: false,
                timer: 2000,
            });
        });
